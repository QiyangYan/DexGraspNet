...
def create_coordinate_axes_traces(origin, rotation, axis_length=0.05, name_prefix='Wrist Base', show_legend=False):
    """
    Build Plotly traces that visualize a coordinate frame located at `origin`
    with orientation described by `rotation`.
    """
    origin = np.asarray(origin, dtype=np.float32).reshape(3)
    rotation = np.asarray(rotation, dtype=np.float32).reshape(3, 3)

    axes_info = [
        ('X', np.array([axis_length, 0.0, 0.0], dtype=np.float32), 'red'),
        ('Y', np.array([0.0, axis_length, 0.0], dtype=np.float32), 'green'),
        ('Z', np.array([0.0, 0.0, axis_length], dtype=np.float32), 'blue'),
    ]

    traces = []
    for idx, (axis_name, direction, color) in enumerate(axes_info):
        end_point = origin + rotation @ direction
        traces.append(
            go.Scatter3d(
    ...
    traces.append(
        go.Scatter3d(
            ...
            name=f'{name_prefix} Origin',
            legendgroup=name_prefix,
            showlegend=show_legend,
            hoverinfo='text',
            hovertext=[f'{name_prefix} Origin']
        )
    )
...
if __name__ == '__main__':
    ...
    for i, idx in enumerate(visualize_indices):
        ...
        base_origin = hand_model.global_translation[i].detach().cpu().numpy()
        base_rotation = hand_model.global_rotation[i].detach().cpu().numpy()
        axis_length = 0.05

        try:
            model_index = i // object_model.batch_size_each
            batch_offset = i % object_model.batch_size_each
            scale_val = object_model.object_scale_tensor[model_index, batch_offset].detach().cpu().item()
            if np.isfinite(scale_val):
                axis_length = max(0.05, float(scale_val) * 0.1)
        except Exception:
            pass

        wrist_axes_traces = create_coordinate_axes_traces(
            origin=base_origin,
            rotation=base_rotation,
            axis_length=axis_length,
            name_prefix='Wrist Base',
            show_legend=(i == 0)
        )
        for trace in wrist_axes_traces:
            fig.add_trace(trace)
