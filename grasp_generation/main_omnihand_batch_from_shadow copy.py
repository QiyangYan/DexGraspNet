...
grasp_file = "dexonomy_1obj_fin_cmap"
...
for i, contact_map_data in enumerate(contact_map_list):
    if len(contact_map_data) > 0:
        ...
        contact_map_goal_parts_tensor = None
        if 'contact_value_parts' in contact_map_data:
            parts_np = contact_map_data['contact_value_parts']
            if isinstance(parts_np, np.ndarray) and parts_np.size > 0:
                contact_map_goal_parts_tensor = torch.from_numpy(parts_np).float().to(device)
        
        # 设置 contact map goal（所有 batch 共享同一个 object）
        hand_model.set_contact_map_goal(contact_map_goal_tensor, contact_map_goal_parts=contact_map_goal_parts_tensor)
...
for step in pbar:
    # Update current step for tensorboard logging
